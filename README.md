<p align="center">
<img src="https://github.com/nerdnam/DDNS-Updater-Python-Ver/blob/main/favicon/favicon.png"  width="300" height="300">
</p>

<p align="center">
<img src="https://github.com/nerdnam/DDNS-Updater-Python-Ver/blob/main/readme/webui.png">
</p>

---
<p align="center">
<img src="https://github.com/nerdnam/DDNS-Updater-Python-Ver/blob/main/readme/homepage_widget.png">
</p>

<p align="center">
<img src="https://github.com/gethomepage/homepage/raw/dev/images/banner_dark@2x.png" width="204" height="51">
</p>
<p align="center">https://github.com/gethomepage/homepage</p>

* homepage dashboard Application examples
```
services.yaml
- DDNS Updater-py:
         icon: /images/icons/ddns-updater.png
         href: http://0.0.0.0:30007
         widget:
           type: customapi
           url: http://0.0.0.0:30007/api?id=home_server
...
*   **Docker 지원**: Docker 및 Docker Compose를 사용하여 쉽게 배포 및 실행 가능.
...
├── static/                                 # 웹 UI용 정적 파일 디렉토리
│   ├── favicon.ico                         # 웹사이트 아이콘 (ICO 형식)
│   ├── favicon.png                         # 웹사이트 아이콘 (PNG 형식)
│   ├── favicon.svg                         # 웹사이트 아이콘 (SVG 형식)
│   ├── favicon.xcf                         # 웹사이트 아이콘 (GIMP XCF 형식)
│   └── styles.css                          # 웹 UI 스타일시트
├── templates/                              # 웹 UI용 HTML 템플릿 디렉토리
│   └── index.html                          # 메인 웹 페이지 템플릿
├── .dockerignore                           # Docker 이미지 생성을 위한 설정 파일
...
미리 빌드된 Docker 이미지를 사용하여 간편하게 실행할 수 있습니다.

**이미지 Pull (선택 사항, `docker run` 또는 `docker-compose up` 시 자동으로 받아옴):**
```bash
docker pull nerdnam/ddns-updater-python-ver:latest
# 예시: docker pull nerdnam/ddns-updater-python-ver:latest
```
(위 `nerdnam/ddns-updater-python-ver:latest`를 실제 Docker Hub에 푸시한 이미지 경로로 변경해주세요. 아직 푸시하지 않았다면 로컬 빌드 이미지 이름을 사용합니다.)
...
docker run -d \
  --name nerdnam/ddns-updater-python-ver:latest \
  -p 40007:40007 \
  ...
  nerdnam/ddns-updater-python-ver:latest 
  # 만약 로컬 빌드 이미지를 사용한다면: ddns-updater-python-ver (또는 빌드 시 사용한 태그)
```
*   `-v $(pwd)/config/ddns_config.ini...`: 호스트의 설정 파일을 컨테이너로 마운트합니다. **반드시 실제 경로로 수정하고, `ddns_config.ini` 파일을 준비해야 합니다.**
*   `-v $(pwd)/data/...`: 상태 파일과 로그를 호스트에 저장하기 위해 마운트합니다.
*   `-e FLASK_SECRET_KEY=...`: **반드시 강력하고 예측 불가능한 값으로 변경하세요.**
*   `nerdnam/ddns-updater-python-ver:latest`: 실제 Docker Hub 이미지 이름 또는 로컬 빌드 이미지 이름으로 변경하세요.
...
services:
  ddns-updater-python-ver: # 서비스 이름
    ...
    image: nerdnam/ddns-updater-python-ver:latest
...
```
(위 로컬 빌드를 사용하려면 `image:` 라인을 주석 처리하고 `build:` 섹션의 주석을 해제하세요.)

**실행 전 준비 (Docker Compose 사용 시):**

*   `docker-compose.yml` 파일이 있는 디렉토리 기준으로 `./config/ddns_config.ini` 파일을 준비합니다.
*   `./data/` 디렉토리를 준비합니다 (상태 파일 및 로그 저장용).

**Docker Compose 실행:**

```bash
docker-compose up -d
# 이미지를 다시 빌드해야 하는 경우: docker-compose up -d --build
```

**로그 확인:**

```bash
docker-compose logs -f ddns-updater-python-ver
```
(서비스 이름을 `docker-compose.yml`에 정의된 대로 사용)
...
```bash
git clone <귀하의_저장소_URL> ddns-updater-python-ver
cd ddns-updater-python-ver
```
( `<귀하의_저장소_URL>` 부분을 실제 저장소 URL로 변경해주세요.)
...
requests>=2.30
pytz>=2024.1
configparser>=5.0  # Python 3.x 에는 내장되어 있지만, 명시적 버전 관리 가능
# 특정 프로바이더에 필요한 라이브러리 (예: boto3, google-auth)
# boto3
# google-auth
# google-auth-httplib2
```
(실제 `requirements.txt`는 `pip freeze > requirements.txt`로 생성된 정확한 버전을 포함해야 합니다.)
...
```json
{
  ...
  "previous_ips": ["000.000.000.000", "000.000.000.000"],
  "date1": "2025-05-07",
  "date2": "25-05-07",
  "time1": "20:30:00",
  "time2": "20:30"
...
(이 목록은 실제 구현된 프로바이더에 따라 업데이트되어야 합니다.)
*   Aliyun
*   All-Inkl
*   ChangeIP
*   Cloudflare
