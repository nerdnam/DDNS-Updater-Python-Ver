# ddns_updater/providers/loopia_provider.py
import requests
...
class LoopiaProvider(BaseProvider):
    NAME = "loopia"
    API_HOST = "dyndns.loopia.se"
    API_PATH = "/" # Loopia는 루트 경로 사용

    def __init__(self, config, logger):
        super().__init__(config, logger)
        self.username = self.config.get('loopia_username')
        self.password = self.config.get('loopia_password')
        ...
        if not all([self.username, self.password, self.domain]):
            error_msg = f"{self.NAME.capitalize()} provider: Missing required configuration (loopia_username, loopia_password, domain)."
            self.logger.error(error_msg)
            raise ValueError(error_msg)
        
        # Go 코드 주석: Loopia says it supports wildcards but cannot get it to work
        if self.config.get('owner') == '*':
            self.logger.warning(f"{self.NAME.capitalize()}: Wildcard owner ('*') is set, but it might not work as expected with Loopia.")
            # Go 코드는 오류 발생시킴. 여기서는 경고만 하고 진행하거나, 동일하게 오류 발생시킬 수 있음.
            # error_msg = f"{self.NAME.capitalize()} provider: Wildcard owner ('*') is not reliably supported."
            # self.logger.error(error_msg)
            # raise ValueError(error_msg)
    ...
    def get_required_config_fields():
        # owner는 Loopia에서 hostname 구성에 필요.
        return ["loopia_username", "loopia_password", "domain", "owner"]

    @staticmethod
    def get_optional_config_fields():
        # Loopia API가 TTL 설정을 지원하는지 확인 필요 (DynDNS v2 표준은 보통 미지원)
        return {"ttl": None} 

    @staticmethod
    def get_description():
        return "Updates DNS records on Loopia (Sweden) using their DynDNS API."
    ...
    def update_record(self, ip_address, record_type="A", proxied=None):
        # Loopia는 IPv4/IPv6 구분 없이 'myip' 파라미터를 사용하고,
        ...
        # URL 구성: https://<username>:<password>@dyndns.loopia.se/
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                # Loopia는 DynDNS 표준 오류 코드(badauth, nohost 등)를 반환할 수 있음.
                if response_text.lower().startswith("nohost") or response_text.lower().startswith("notfqdn"):
                    error_message = "API Error: Hostname not found or not a FQDN."
                elif response_text.lower().startswith("badrequest") or response_text.lower().startswith("911"):
                     error_message = "API Error: Bad request or system error (911)."
            ...
            if response_text.lower().startswith("nohost") or response_text.lower().startswith("notfqdn"):
                ...
                return False, "API Error: Bad request or system error (911)."
            elif response_text.lower().startswith("badauth"):
                ...
                # Loopia 응답은 "good <IP Address>" 또는 "nochg <IP Address>" 형태일 수 있음.
                # IP 추출 및 비교 로직 추가 권장.
