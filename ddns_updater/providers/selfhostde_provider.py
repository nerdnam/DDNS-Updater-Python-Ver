# ddns_updater/providers/selfhostde_provider.py
import requests
from urllib.parse import quote # URL에 사용자 이름/비밀번호 포함 시 필요할 수 있음

from .base_provider import BaseProvider

class SelfhostdeProvider(BaseProvider):
    NAME = "selfhostde"
    API_HOST = "carol.selfhost.de"
    API_PATH = "/nic/update"

    def __init__(self, config, logger):
        super().__init__(config, logger)
        self.username = self.config.get('selfhostde_username')
        self.password = self.config.get('selfhostde_password')
        # self.domain, self.owner 는 BaseProvider 에서 처리
        
        # owner 기본값 설정
        if not self.config.get('owner'):
            self.config['owner'] = '@'
            self.logger.debug(f"{self.NAME.capitalize()}: 'owner' not set, defaulting to '@'.")

        # 필수 설정값 확인
        if not all([self.username, self.password, self.domain]):
            error_msg = f"{self.NAME.capitalize()} provider: Missing required configuration (selfhostde_username, selfhostde_password, domain)."
            self.logger.error(error_msg)
            raise ValueError(error_msg)
        
        if self.config.get('owner') == '*':
            error_msg = f"{self.NAME.capitalize()} provider: Wildcard owner ('*') is not allowed."
            self.logger.error(error_msg)
            raise ValueError(error_msg)


    @staticmethod
    def get_required_config_fields():
        # owner는 Selfhost.de에서 hostname 구성에 필요.
        return ["selfhostde_username", "selfhostde_password", "domain", "owner"]

    @staticmethod
    def get_optional_config_fields():
        # Selfhost.de API가 TTL 설정을 지원하는지 확인 필요 (DynDNS v2 표준은 보통 미지원)
        return {"ttl": None} 

    @staticmethod
    def get_description():
        return "Updates DNS records on Selfhost.de using their DynDNS API."

    def _build_hostname_for_query(self):
        """Go 코드의 utils.BuildURLQueryHostname과 유사하게 호스트명 구성"""
        owner = self.config.get('owner', '@')
        if owner == '@' or owner == '' or owner is None:
            return self.domain
        return f"{owner}.{self.domain}"

    def update_record(self, ip_address, record_type="A", proxied=None):
        # Selfhost.de는 IPv4/IPv6 구분 없이 'myip' 파라미터를 사용하고,
        # record_type (A/AAAA)은 API 요청에 직접 사용되지 않음.
        if proxied is not None:
            self.logger.warning(f"{self.NAME.capitalize()} provider does not use the 'proxied' option. It will be ignored.")

        hostname_for_query = self._build_hostname_for_query()
        
        self.logger.info(f"{self.NAME.capitalize()}: Attempting to update {hostname_for_query} (record type {record_type}) to IP: {ip_address}")

        # URL 구성: https://<username>:<password>@carol.selfhost.de/nic/update
        userinfo_user = quote(self.username, safe='')
        userinfo_pass = quote(self.password, safe='')
        
        target_url = f"https://{userinfo_user}:{userinfo_pass}@{self.API_HOST}{self.API_PATH}"
        
        params = {
            'hostname': hostname_for_query,
            'myip': ip_address
        }
        
        headers = {'User-Agent': f'Python-DDNS-Updater/{self.NAME}'}
        timeout = self.config.get('http_timeout_seconds', 10)

        try:
            response = requests.get(target_url, params=params, headers=headers, timeout=timeout)
            
            response_text = response.text.strip() if response.text else ""
            self.logger.debug(f"{self.NAME.capitalize()} API Response Status: {response.status_code}, Body: '{response_text}'")

            # HTTP 상태 코드 우선 처리 (Go 코드 참조)
            if response.status_code == 200: # OK
                # 상태 코드 200일 때 응답 본문 분석
                if response_text.lower().startswith("notfqdn"):
                    return False, "API Error: Hostname is not a FQDN or does not exist (notfqdn)."
                elif response_text.lower().startswith("abuse"):
                    return False, "API Error: Account blocked for abuse."
                elif response_text.lower().startswith("badrequest"):
                    return False, "API Error: Bad request."
                elif response_text.lower().startswith("good") or response_text.lower().startswith("nochg"):
                    # 성공. 요청한 IP로 업데이트되었다고 가정.
                    success_message = f"Successfully updated {hostname_for_query} to {ip_address} (assumed)."
                    if response_text:
                         success_message += f" API Response: '{response_text}'"
                    self.logger.info(success_message)
                    return True, success_message
                else:
                    return False, f"API Error: Unknown success response (200 OK): '{response_text}'"
            elif response.status_code == 204: # No Content (no change)
                self.logger.info(f"IP address {ip_address} for {hostname_for_query} is already up to date (API status 204 No Content).")
                return True, f"IP address already up to date (204 No Content). API Response: '{response_text}'"
            elif response.status_code == 401: # Unauthorized
                return False, f"API Error: Authentication failed (HTTP 401). Response: '{response_text}'"
            elif response.status_code == 409: # Conflict (Zone not found by Go code)
                return False, f"API Error: Zone not found or conflict (HTTP 409). Response: '{response_text}'"
            elif response.status_code == 410: # Gone (Account inactive)
                return False, f"API Error: Account inactive (HTTP 410). Response: '{response_text}'"
            elif response.status_code == 411: # Length Required (IP Sent Malformed by Go code)
                return False, f"API Error: IP address sent was malformed or missing (HTTP 411). IP: {ip_address}. Response: '{response_text}'"
            elif response.status_code == 412: # Precondition Failed (Private IP Sent)
                return False, f"API Error: Private IP address sent (HTTP 412). IP: {ip_address}. Response: '{response_text}'"
            elif response.status_code == 503: # Service Unavailable (DNS Server Side)
                return False, f"API Error: DNS server-side issue (HTTP 503). Response: '{response_text}'"
            else: # 그 외 다른 오류 상태 코드
                error_message = f"API Error: HTTP {response.status_code}"
                if response_text: error_message += f" - {response_text}"
                return False, error_message

        except requests.exceptions.RequestException as e:
            self.logger.error(f"{self.NAME.capitalize()} API request failed: {e}")
            return False, f"API Request Error: {e}"